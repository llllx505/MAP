...
class ResidualAttentionBlock_IVLP(nn.Module):
    ...
    def task_forward(self, x: torch.Tensor,t_p):
        # Will need to append the learnable tokens for this layer here
        # Check if flag was set for this layer or not
        if self.add_prompt:
            # Also see if this is textual transformer layer or not
            if not self.text_layer:
                ...
                suffix = x[1 + self.n_ctx_text:, :, :]
                # Create/configure learnable tokens of this layer
                ctx=self.VPT_shallow+t_p
                textual_context = ctx.expand(x.shape[1], -1, -1).permute(1, 0, 2).half()
                # Add the learnable tokens of this layer with the input, replaced by previous
                # layer learnable tokens
                x = torch.cat([prefix, textual_context, suffix], dim=0)
            ...
            if len(compound_prompts_deeper) > 0:
                ...
                if not self.text_layer:
                    ...
                    if not (counter > len(compound_prompts_deeper) - 1):
                        ...
                        # x -> [77, NCLS, DIM]
                        # First remove the learnable tokens from previous layer
                        prefix = x[:1, :, :]#torch.Size([1, 40, 512])
                        suffix = x[1 + self.compound_prompt_nctx:, :, :]#torch.Size([67, 40, 512])
                        # Create/configure learnable tokens of this layer
                        textual_context = compound_prompts_deeper[counter]#torch.Size([4, 4, 512])
                        n_txt=textual_context.shape[1]
                        n_cls=x.shape[1]//4
                        textual_context=textual_context.expand(n_cls, -1, -1,-1)
                        textual_context=textual_context.permute(1,0,2,3)
                        textual_context=textual_context.contiguous().view(4*n_cls,n_txt,x.shape[-1])
            ...
            if len(compound_prompts_deeper) > 0:
                ...
                if not self.text_layer:
                    ...
                    if not (counter > len(compound_prompts_deeper) - 1):
                        # Remove the outputs produced by learnable tokens of previous layer
                        prefix = x[0:x.shape[0] - self.compound_prompt_nctx, :, :]
                        postfix=x[x.shape[0] - self.compound_prompt_nctx, :, :]
                        # Create/configure learnable tokens of this layer
                        visual_context = compound_prompts_deeper[counter]  # extract the correct index
                        visual_context = visual_context.expand(x.shape[1], -1, -1).permute(1, 0, 2).half()
                        # Add the learnable tokens of this layer with the input, by replacing previous
                        # layer learnable tokens
                        x = torch.cat([prefix, visual_context+postfix], dim=0)
                    ...
                    if not (counter > len(compound_prompts_deeper) - 1):
                        ...
                        # First remove the learnable tokens from previous layer
                        prefix = x[:1+4, :, :]
                        suffix = x[1+4 + self.compound_prompt_nctx:, :, :]
                        midfix=x[1:1 + self.compound_prompt_nctx, :, :]
                        # Create/configure learnable tokens of this layer
...
class VisionTransformer(nn.Module):
    ...
    def task_forward(self, x: torch.Tensor):
        x = self.conv1(x)  # shape = [*, width, grid, grid]
        x = x.reshape(x.shape[0], x.shape[1], -1)  # shape = [*, width, grid ** 2]
        x = x.permute(0, 2, 1)  # shape = [*, grid ** 2, width]
        x = torch.cat(
        ...
        x = x.permute(1, 0, 2)  # NLD -> LND
        x = self.transformer.task_forward(x)
        x = x.permute(1, 0, 2)  # LND -> NLD
...
class VisionTransformer_MaPLe(nn.Module):
    ...
    def forward(self, x: torch.Tensor, shared_ctx, compound_deeper_prompts):
        x = self.conv1(x)  # shape = [*, width, grid, grid]
        x = x.reshape(x.shape[0], x.shape[1], -1)  # shape = [*, width, grid ** 2] 
        x = x.permute(0, 2, 1)  # shape = [*, grid ** 2, width] torch.Size([4, 196, 768])
        x = torch.cat(
            [self.class_embedding.to(x.dtype) + torch.zeros(x.shape[0], 1, x.shape[-1], dtype=x.dtype, device=x.device),
             x], dim=1)  # shape = [*, grid ** 2 + 1, width]
        x = x + self.positional_embedding.to(x.dtype) #torch.Size([4, 197, 768])

        # After positional embeddings, we will attach prompts with the model, remember only those
        # are trainable parameters here in whole image encoder.
        if self.VPT_shallow:
            visual_ctx = shared_ctx.expand(x.shape[0], -1, -1).half()
            x = torch.cat([x, visual_ctx], dim=1) #torch.Size([4, 199, 768])
        else:
            assert self.prompt_till_layer_visual == 0
    ...
    def task_forward(self, x: torch.Tensor, shared_ctx, compound_deeper_prompts):
        x = self.conv1(x)  # shape = [*, width, grid, grid]
        x = x.reshape(x.shape[0], x.shape[1], -1)  # shape = [*, width, grid ** 2] 
        x = x.permute(0, 2, 1)  # shape = [*, grid ** 2, width] torch.Size([4, 196, 768])
        x = torch.cat(
        ...
        x = x.permute(1, 0, 2)  # NLD -> LND
        # Again combine the inputs, so nn.sequential can work
        outputs = self.transformer.task_forward([x, compound_deeper_prompts, 0])  # third argument is counter
        x = outputs[0]
        x = x.permute(1, 0, 2)  # LND -> NLD
